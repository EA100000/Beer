#!/usr/bin/env python3
"""
Analyse approfondie des 230,558 matchs pour amliorer les prdictions
...
print("ç ANALYSE APPROFONDIE DES DONNES RELLES")
print("=" * 80)
print()

# Charger les donnes
print("Ç Chargement de Matches.csv (230,558 matchs)...")
df = pd.read_csv('Matches.csv')
print(f" {len(df):,} matchs chargs")
print()

# Afficher les colonnes disponibles
print("ã COLONNES DISPONIBLES :")
print(df.columns.tolist())
print()

# Filtrer les matchs avec donnes compltes
print("π Filtrage des matchs avec donnes compltes...")
required_cols = ['HomeFouls', 'AwayFouls', 'HomeCorners', 'AwayCorners',
                 'HomeYellow', 'AwayYellow']

df_clean = df.dropna(subset=required_cols)
print(f" {len(df_clean):,} matchs avec donnes compltes ({len(df_clean)/len(df)*100:.1f}%)")
print()
...
# ============================================================================
print("=" * 80)
print("ä ANALYSE 1 : STATISTIQUES DESCRIPTIVES")
print("=" * 80)
print()
...
for market, col in [('Fautes', 'TotalFouls'),
    ...
    print(f"  Mdiane        : {data.median():.2f}")
    print(f"  cart-type     : {data.std():.2f}")
...
# ============================================================================
# ANALYSE 2 : CORRLATIONS
# ============================================================================
print("\n" + "=" * 80)
print("ó ANALYSE 2 : CORRLATIONS ENTRE VARIABLES")
print("=" * 80)
print()

correlation_cols = ['HomeFouls', 'AwayFouls', 'HomeCorners', 'AwayCorners',
                    'HomeYellow', 'AwayYellow', 'TotalFouls', 'TotalCorners',
                    'TotalYellow']

if 'HomeTarget' in df_clean.columns and 'AwayTarget' in df_clean.columns:
    correlation_cols.extend(['HomeTarget', 'AwayTarget'])

corr_matrix = df_clean[correlation_cols].corr()

print("ä Corrlations importantes (|r| > 0.3) :")
print()
...
for i in range(len(corr_matrix.columns)):
    for j in range(i+1, len(corr_matrix.columns)):
        ...
        if abs(corr) > 0.3:
            ...
            correlations_dict[f"{var1}_{var2}"] = round(corr, 3)

# ============================================================================
# ANALYSE 3 : PATTERNS DOMICILE/EXTRIEUR
# ============================================================================
print("\n" + "=" * 80)
print("† ANALYSE 3 : AVANTAGE DOMICILE/EXTRIEUR")
print("=" * 80)
print()
...
for market, home_col, away_col in [('Fautes', 'HomeFouls', 'AwayFouls'),
    ...
    print(f"  Extrieur : {away_mean:.2f} en moyenne")
    print(f"  Diffrence : {diff_pct:+.1f}%")
...
# ============================================================================
# ANALYSE 4 : VARIANCE ET STABILIT
# ============================================================================
print("\n" + "=" * 80)
print("à ANALYSE 4 : VARIANCE ET STABILIT (COEFFICIENT DE VARIATION)")
print("=" * 80)
print()
...
for market, col in [('Fautes', 'TotalFouls'),
    ...
    print(f"  cart-type     : {std:.2f}")
    print(f"  Coef. Variation: {cv:.1f}%")
    print(f"  Stabilit      : {'TRS STABLE' if cv < 20 else 'STABLE' if cv < 30 else 'VOLATILE'}")

    variance_dict[market] = {
        'mean': round(mean, 2),
        'std': round(std, 2),
        'cv': round(cv, 2),
        'stability': 'TRS STABLE' if cv < 20 else 'STABLE' if cv < 30 else 'VOLATILE'
    }

# ============================================================================
# ANALYSE 5 : MEILLEURS SEUILS (MAXIMISATION DU WIN RATE)
# ============================================================================
print("\n" + "=" * 80)
print("Ø ANALYSE 5 : MEILLEURS SEUILS POUR MAXIMISER LE WIN RATE")
print("=" * 80)
print()
...
for market, col in [('Fautes', 'TotalFouls'),
    ...
    # Trouver les seuils qui donnent le meilleur win rate
    # On cherche les seuils o la distance  la moyenne est maximale
    ...
    for threshold in test_thresholds:
        ...
        # Meilleurs seuils = ceux o le win rate est le plus loign de 50%
...
# ============================================================================
print("\n" + "=" * 80)
print("æ SAUVEGARDE DES RSULTATS")
print("=" * 80)
print()
...
print(" Rsultats sauvegards dans : real_data_analysis.json")
print()
...
# ============================================================================
print("=" * 80)
print("Ø RECOMMANDATIONS POUR AMLIORER LES ALGORITHMES")
print("=" * 80)
print()

# Calculer les ajustements recommands
...
print("1 COEFFICIENTS DE VARIATION ( utiliser dans enhancedOverUnder.ts) :")
...
print("2 AJUSTEMENTS DOMICILE/EXTRIEUR ( utiliser) :")
print(f"   Fautes : Domicile {fautes_home_boost:+.1%}, Extrieur {-fautes_home_boost:+.1%}")
print(f"   Corners : Domicile {corners_home_boost:+.1%}, Extrieur {-corners_home_boost:+.1%}")
print()
...
print(" Recommandations sauvegardes dans : recommendations.json")
print()
...
print("=" * 80)
print()
print("Å Fichiers gnrs :")
print("   - real_data_analysis.json : Statistiques dtailles")
print("   - recommendations.json : Recommandations d'amlioration")
print()
print("Ä Prochaine tape : Intgrer ces insights dans enhancedOverUnder.ts")
